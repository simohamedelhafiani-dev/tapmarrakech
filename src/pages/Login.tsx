          ...
          <div className="w-full rounded-[2rem] border border-ink/5 bg-white p-7 shadow-2xl md:p-9">
            ...
            <div className="mb-8">
              <div className="mt-7 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gold">Connexion</p>
                  <h1 className="font-display text-2xl text-forest">{isAdmin ? 'Administrateur' : 'Responsable'}</h1>
                </div>
              </div>
            </div>
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-4rem)] max-w-5xl items-center justify-center">
        <div className="relative z-10 w-full">
          <div className="mx-auto mb-10 max-w-2xl text-center">
            <p className="mt-5 text-xs font-semibold uppercase tracking-[0.24em] text-gold">Espace professionnel</p>
            <h1 className="mt-3 font-display text-3xl text-forest md:text-5xl">Comment souhaitez-vous accéder à TapMarrakech ?</h1>
            <p className="mx-auto mt-4 max-w-xl text-sm leading-6 text-ink/45 md:text-base">Choisissez votre profil. Chaque accès ouvre uniquement les fonctions qui lui sont destinées.</p>
          </div>
          <div className="grid gap-5 md:grid-cols-3">
            <button onClick={() => selectRole('admin')} className="group rounded-[2rem] border border-ink/5 bg-white p-7 text-left shadow-sm transition duration-200 hover:-translate-y-1 hover:border-forest/20 hover:shadow-xl">
              <div className="flex items-start justify-between">
                <div className="grid h-14 w-14 place-items-center rounded-2xl bg-forest text-white shadow-lg"><ShieldCheck size={27} /></div>
                <ArrowRight size={19} className="text-ink/20 transition group-hover:translate-x-1 group-hover:text-forest" />
              </div>
              <p className="mt-8 text-[10px] font-semibold uppercase tracking-[0.2em] text-gold">Accès principal</p>
              <h2 className="mt-2 font-display text-3xl text-forest">Admin</h2>
              <p className="mt-3 text-sm leading-6 text-ink/45">Administration de TapMarrakech, établissements, équipes et configuration.</p>
              <p className="mt-6 text-xs font-semibold text-forest">Email + mot de passe</p>
            </button>
            <button onClick={() => selectRole('responsible')} className="group rounded-[2rem] border border-ink/5 bg-white p-7 text-left shadow-sm transition duration-200 hover:-translate-y-1 hover:border-forest/20 hover:shadow-xl">
              <div className="flex items-start justify-between">
                <div className="grid h-14 w-14 place-items-center rounded-2xl bg-[#f4ead3] text-forest shadow-sm"><UserRound size={27} /></div>
                <ArrowRight size={19} className="text-ink/20 transition group-hover:translate-x-1 group-hover:text-forest" />
              </div>
              <p className="mt-8 text-[10px] font-semibold uppercase tracking-[0.2em] text-gold">Gestion établissement</p>
